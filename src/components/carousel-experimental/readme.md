# [Carousel Experimental](https://winduum.dev/docs/components/carousel-experimental.html)

## Installation
By default, we recommend copying and pasting the code from this directory to your project and remapping the imports to local.

```js
import { Carousel } from '@/components/ui/carousel-experimental'
```
### NPM
Alternatively, you can also import directly from `npm` so you can leverage updates.
```shell
npm i winduum-vue
```

```js
import { Carousel } from 'winduum-vue/src/components/carousel-experimental'
```

### Docs
Visit [docs](https://winduum.dev/docs/components/carousel-experimental.html) to learn more and see usage examples.
